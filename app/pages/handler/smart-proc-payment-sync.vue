...
onMounted(async () =>
{
	...
	catch( error )
	{
		...
		processError(
			(new Result()).addError(error as Error),
			error as Error
		)
	}
})
...
const loadEntityData = async (): Promise<void> =>
{
	...
	try
	{
		...
		const commands = {
			...
			getBankPaymentDistributions: {
				...
				params: {
					...
					select: [
						'id', 'ufCrm13PaymentId', 'parentId1036', 'parentId2',
		...
		if(result.isSuccess)
		{
			...
			{
				result.addError(new Error(
					'Стоит заполнить поле [Реквизиты вашей компании].'
				))
			}
			else
			{
		...
		if(result.isSuccess)
		{
			...
			{
				result.addError(new Error(
					'Стоит в поле [Клиент] указать того кто платил.'
				))
			}
			else
			{
...
const isDistributionsSumWarning = computed(() =>
{
	...
	if (distributionsSum.value > entity.value.needDistributionsSum) {
		result.addError(new Error(
			'Вы хотите распределить сумму большую чем требуется'
		))
	}
...
const makeOpenSliderDeal = async (entityId: number) =>
{
	return $b24.slider.openPath(
		$b24.slider.getUrl(`/crm/deal/details/${ entityId }/`),
		950
	)
}
// endregion ////
	...
	<ClientOnly>
		...
		>
			<div :class="{ 'overflow-hidden': !isInit }">
				<div v-if="!isInit || isProcess" class="w-full h-screen flex flex-col items-center justify-center">
				...
				<template v-else>
					...
					>
						...
						<div class="font-b24-secondary flex flex-col items-start justify-between gap-1">
							...
							<B24Badge use-fill color="ai" depth="dark">
								<template #default>
									{{ b24Helper?.currency.format(dealRow.opportunity, dealRow.currencyId, b24CurrentLang).replaceAll('&amp;nbsp;', ' ') }}
								</template>
							</B24Badge>
						</div>
						<div class="mb-3 w-full grid grid-cols-[repeat(auto-fill,minmax(266px,1fr))] gap-y-sm gap-x-xs">
							<div
								...
								:class="payment.paid === 'Y' ? 'border-success-text' : 'border-base-50 hover:border-primary'"
							>
								<div
									v-if="payment.paid === 'Y'"
								...
								<div class="max-w-11/12">
									<div class="font-b24-secondary text-black text-h6 leading-4 mb-xs font-semibold line-clamp-2">
										{{payment.paySystemName}} {{payment.accountNumber}} <small
										class="text-gray-500">[id: {{payment.id}}]</small>
									</div>
									<div class="font-b24-primary text-sm line-clamp-6">
										<div class="font-semibold">
											{{ b24Helper?.currency.format(payment.sum, payment.currency, b24CurrentLang).replaceAll('&amp;nbsp;', ' ') }}
										</div>
					...
					<div class="mb-[250px] " />
					<div class="fixed bg-white min-h-[60px] bottom-0 w-full shadow-top-sm pr-(--scrollbar-width)">
						<div class="px-4 relative w-full flex flex-row flex-nowrap items-center justify-between gap-2">
							...
							<B24DescriptionList
								...
								:items="[
									{
										label: 'Всего:',
										description: b24Helper?.currency.format(entity.opportunity, entity.currencyId, b24CurrentLang).replaceAll('&amp;nbsp;', ' '),
									...
									},
									{
										...
										description: b24Helper?.currency.format(entity.needDistributionsSum, entity.currencyId, b24CurrentLang).replaceAll('&amp;nbsp;', ' ')
									},
									{
										label: 'Вы хотите распределить:',
										description: b24Helper?.currency.format(distributionsSum, entity.currencyId, b24CurrentLang).replaceAll('&amp;nbsp;', ' '),
										orientation: 'horizontal',
										actions: [
											{
												label: 'Распределить',
